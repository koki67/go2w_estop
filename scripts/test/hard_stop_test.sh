...
ros2 launch go2w_estop_robot robot_service.launch.py >/tmp/go2w_estop_hard_robot.log 2>&1 &
ROBOT_PID=$!
sleep 3

ros2 topic pub --once /estop/hard_stop std_msgs/msg/Empty "{}" >/dev/null
sleep 1

STATUS_OUTPUT="$(timeout 10 ros2 topic echo --once /estop/status)"
printf '%s\n' "$STATUS_OUTPUT" | grep -q "current_state: STOPPED_LATCHED"
printf '%s\n' "$STATUS_OUTPUT" | grep -q "last_trigger_type: HARD"
printf '%s\n' "$STATUS_OUTPUT" | grep -q "latched: true"

echo "PASS: hard_stop_test"
