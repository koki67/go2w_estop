...
ros2 launch go2w_estop_robot robot_service.launch.py >/tmp/go2w_estop_smoke_robot.log 2>&1 &
ROBOT_PID=$!
sleep 3

ros2 topic list | grep -q "/estop/status"
ros2 topic list | grep -q "/estop/protective_stop"
ros2 topic list | grep -q "/estop/hard_stop"

timeout 10 ros2 topic echo --once /estop/status | grep -q "current_state: READY"
...
RATE_LINE="$(timeout 8 ros2 topic hz /api/sport/request --window 10 2>&1 | grep "average rate" | tail -n 1)"
RATE_VALUE="$(printf '%s\n' "$RATE_LINE" | awk '{print $3}')"
awk -v rate="$RATE_VALUE" 'BEGIN { exit !(rate >= 4.0 && rate <= 6.0) }'
